...
export function getMatchEditData(
  state: CorrectSolutionViewState,
  sampleSolution: SolutionNodeFragment[],
  onDeleteMatch: (sampleValue: number, userValue: number) => void
): MatchEditData | undefined {
  ...
  return matches.length > 0
    ? { markedNodeSide, markedNode, matches, onDeleteMatch }
    : undefined;
}
