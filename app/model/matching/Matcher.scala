...
trait Matcher[T, M <: Match[T], MR <: MatchingResult[T, M]] {
  ...
  protected def findUserSolutionMatch(
    sampleSolutionNode: T,
    userSolution: List[T]
  ): Option[(T, List[T])] = {
  ...
  def performMatching(sampleSolution: Seq[T], userSolution: Seq[T]): MR = {
    ...
    @tailrec
    def go(remainingSampleNodes: List[T], remainingUserNodes: List[T], matches: Seq[M], notMatchedSample: Seq[T]): MR =
