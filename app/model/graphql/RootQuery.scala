...
trait RootQuery extends GraphQLBasics:
  ...
  private val resolveReviewCorrection: Resolver[Unit, ReviewData] = resolveWithUser { case (context, user) =>
    ...
    for {
      ...
      userSolutionNodes   <- context.ctx.tableDefs.futureUserSolNodesForUserSolution(username, exerciseId)
      sampleSolutionNodes <- context.ctx.tableDefs.futureSampleSolNodesForExercise(exerciseId)
      ...
      (_, CorrectionSummary(comment, points)) <- futureFromOption(maybeCorrectionSummary, UserFacingGraphQLError("Correction summary not found!"))
  ...
  protected val queryType: ObjectType[GraphQLContext, Unit] = ObjectType(
    ...
    fields[GraphQLContext, Unit](
      ...
      Field("abbreviations", ListType(Abbreviation.queryType), resolve = resolveAbbreviations),
